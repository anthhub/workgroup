...
func TestBasic(t *testing.T) {
	...
	defer func() {
		// cause panic, workgroup deferred cancel will be called.
		// newArr will be [1,2,3,0].
		assert.Equal(t, len(newArr), 4)

		time.Sleep(10 * time.Millisecond)
		// workgroup cancel will terminal all workers.
	...
	for i := range arr {
		...
		if err != nil {
			...
			// panic will be recovered by workgroup, and return a error.
			assert.Contains(t, err.Error(), "workgroup: panic recovered:")
			return
		}
...
func TestLimit(t *testing.T) {
	...
	for i := range arr {
		...
		// v will be 5, 4, 3, 2, 1.
		// cause workgroup limit is 1, so tasks will be executed one by one.
		assert.Equal(t, arr[i], v)
	}
}

// workgroup manual cancel test
